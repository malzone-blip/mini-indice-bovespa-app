...
def get_intraday_data(symbol='PETR4.SA', interval='15min'):
    url = f'https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol={symbol}&interval={interval}&apikey={ALPHA_VANTAGE_KEY}&outputsize=compact'
    ...
    if response.status_code == 200:
        ...
        if key in data:
            ...
            st.error(f"Erro: '{key}' não encontrado na resposta da API")
            return None
    else:
        st.error(f"Erro HTTP {response.status_code} na requisição Alpha Vantage (intraday)")
        return None

def get_daily_data(symbol='PETR4.SA'):
    ...
    if response.status_code == 200:
        ...
        st.error(f"Erro HTTP {response.status_code} na requisição Alpha Vantage (daily)")
        return None
...
def main():
    st.title('Monitoramento Mini Índice Bovespa - Sinais Automáticos com Alpha Vantage Intraday')
    st.write('Informe o símbolo e selecione o intervalo para análise intraday ou diário.')

    symbol = st.text_input('Símbolo (ex: PETR4.SA)', 'PETR4.SA').upper()
    interval = st.selectbox('Intervalo da análise', ['Diário', '1min', '5min', '15min', '30min', '60min'])

    if st.button('Atualizar Dados e Sinais'):
        with st.spinner('Buscando dados e gerando sinais...'):
            if interval == 'Diário':
                df = get_daily_data(symbol)
            else:
                intraday_interval = interval.lower()
                df = get_intraday_data(symbol, interval=intraday_interval)
            price, last_trade = get_current_price(symbol)

            if price:
                st.markdown(f"### Preço atual: **R$ {price:.2f}** (baseado em {last_trade})")

            if df is not None and not df.empty:
                df = calculate_indicators(df)
                df = generate_trade_signals(df)

                st.subheader(f'Últimos dados e sinais - {symbol}')
                st.dataframe(df[['date', 'close', 'EMA9', 'EMA21', 'RSI14', 'UpperBB', 'LowerBB', 'Signal']].tail(20))

                st.markdown("""
                **Legenda das colunas:**  
                - `date`: Data e hora da cotação.  
                - `close`: Preço de fechamento do período.  
                - `EMA9`: Média móvel exponencial dos últimos 9 períodos.  
                - `EMA21`: Média móvel exponencial dos últimos 21 períodos.  
                - `RSI14`: Índice de força relativa calculado em 14 períodos.  
                - `UpperBB`: Banda superior de Bollinger (limite máximo esperado).  
                - `LowerBB`: Banda inferior de Bollinger (limite mínimo esperado).  
