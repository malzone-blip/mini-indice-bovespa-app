...
def get_historical_data(symbol='PETR4', range_days='7d'):
    ...
    if response.status_code == 200:
        data = response.json()
        st.write("Dados brutos API BRAPI:", data)  # mostrar JSON completo para debug
        if 'results' in data and len(data['results']) > 0:
            df = pd.DataFrame(data['results'])
            st.write("DataFrame inicial:", df.head())  # mostrar primeiras linhas para confirmar colunas
            # Ajuste os nomes das colunas conforme o retorno real da API BRAPI
            # Por exemplo, pode precisar mudar 'close' para outro nome
            # Exemplo provisório:
            if 'close' not in df.columns and 'closePrice' in df.columns:
...
def calculate_indicators(df):
    if df is not None and not df.empty:
        ...
        df['Volume'] = np.nan  # Volume pode estar indisponível na API
    return df
