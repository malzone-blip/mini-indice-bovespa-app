...
def get_historical_data(symbol):
    data_sources = [
        (get_data_alpha_vantage, 'Alpha Vantage'),
        (get_data_brapi, 'BRAPI'),
        (get_data_finnhub, 'Finnhub'),
        (get_data_twelvedata, 'Twelve Data')
    ]
    for func, name in data_sources:
        df = None
        try:
            df = func(symbol)
            if df is not None and not df.empty:
                st.write(f"Dados obtidos via {name}")
                return df
        except Exception as e:
            st.warning(f"Erro obtendo dados de {name}: {e}")
    st.error("Não foi possível obter dados históricos de nenhuma fonte.")
    return None
...
def main():
    st.title('Monitoramento Mini Índice Bovespa - Dados multi-API e Sinais Automáticos')
    symbol = st.text_input('Símbolo (ex: PETR4.SA)', 'PETR4.SA').upper()

    if st.button('Atualizar Dados e Sinais'):
        with st.spinner('Obtendo dados de múltiplas fontes...'):
            df = get_historical_data(symbol)
            if df is not None:
                df = calculate_indicators(df)
                df = generate_trade_signals(df)
                st.subheader(f'Dados e sinais para {symbol}')
