...
def get_quote(symbol='PETR4.SA'):
    ...
    if response.status_code == 200:
        ...
        if 'Global Quote' in data:
            ...
            # O horário exato da última atualização nao é disponibilizado, só o dia 
            return price, latest_trading_day
        else:
...
def main():
    ...
    st.write('Informe o símbolo da ação (ex: PETR4.SA) e o período para análise.')

    symbol = st.text_input('Ativo (símbolo)', 'PETR4.SA').upper()
    period = st.selectbox('Período para dados históricos', ['7d', '1mo', '3mo', '6mo', '1y'], index=0)

    if st.button('Atualizar Dados e Sinais'):
        with st.spinner('Consultando dados e processando...'):
            # Buscar dados históricos
            df = get_historical_data(symbol)
            # Buscar preço atual e data
            price, last_day = get_quote(symbol)

            if df is not None and not df.empty:
                df = calculate_indicators(df)
                df = generate_trade_signals(df)

                # Mostrar preço atual com delay aproximado (considerando dia da última atualização)
                st.markdown(f"### Preço atual de {symbol}: **R$ {price:.2f}** (última atualização em {last_day})")
                ...
                # Legenda explicativa das colunas
                st.markdown("""
                **Legenda das colunas:**  
                - `date`: Data da cotação.  
                - `close`: Preço de fechamento do dia.  
                - `EMA9`: Média móvel exponencial de 9 dias.  
                - `EMA21`: Média móvel exponencial de 21 dias.  
                - `RSI14`: Índice de Força Relativa calculado em 14 dias.  
                - `UpperBB`: Banda superior de Bollinger (limite superior).  
                - `LowerBB`: Banda inferior de Bollinger (limite inferior).  
                - `Signal`: Sinal gerado para operações: `BUY` (comprar), `SELL` (vender), `HOLD` (manter posição).
                """)
