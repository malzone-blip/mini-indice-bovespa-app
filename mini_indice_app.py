...
def get_historical_data(symbol='PETR4', range_days='7d'):
    ...
    st.write(f"Tamanho da resposta: {len(response.text)} caracteres")
    try:
        data = response.json()
    except Exception as e:
        st.error(f"Erro ao decodificar JSON da resposta API: {e}")
        st.text(f"Conteúdo bruto da resposta:\n{response.text}")
        return None
    st.write("Dados brutos da API BRAPI (histórico):", data)
...
# Mantém as outras funções calcular indicadores e gerar sinais igual ao anterior
