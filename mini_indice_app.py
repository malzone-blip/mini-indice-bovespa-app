        ...
        with st.spinner('Buscando dados e processando...'):
            ...
            if df is not None and not df.empty:
                ...
                **Legenda:**  
                ...
                csv_buffer = StringIO()
                df.tail(20).to_csv(csv_buffer, index=False)
                st.download_button(label="Baixar relatório em CSV", data=csv_buffer.getvalue(),
                                   file_name=f"relatorio_{symbol}.csv", mime="text/csv")
            else:
