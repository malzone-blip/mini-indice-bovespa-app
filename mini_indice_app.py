...
def generate_pdf(df, symbol):
    ...
    c.drawString(30, height - 30, f"Relatório de dados e sinais para {symbol}")
    ...
    for i, row in df.iterrows():
        ...
        text = f"{row['date'].strftime('%Y-%m-%d')} Close: {row['close']:.2f} Signal: {row['Signal']}"
...
def main():
    st.title('Monitoramento Alpha Vantage - Mini Índice Brasileiro')
