...
st.set_page_config(page_title="Monitoramento Alpha Vantage", layout="wide", initial_sidebar_state="collapsed")
...
def generate_pdf(df, symbol):
    buffer = BytesIO()
    c = canvas.Canvas(buffer, pagesize=letter)
    width, height = letter
    c.setFont("Helvetica-Bold", 14)
    c.drawString(30, height - 40, f"Relatório Financeiro para {symbol}")
    ...
    for i, row in df.iterrows():
        if y < 50:
            ...
            c.setFont("Helvetica", 10)
            y = height - 50
        text = f"{row['date'].strftime('%Y-%m-%d')} | Fechamento: R$ {row['close']:.2f} | Sinal: {row['Signal']}"
    ...
    if st.button('Atualizar Dados e Sinais'):
        with st.spinner('Buscando dados e processando...'):
            ...
            if df is not None and not df.empty:
                ...
                pdf_buffer = generate_pdf(df.tail(20), symbol)
                st.download_button(label="Baixar relatório em PDF", data=pdf_buffer,
                                   file_name=f"relatorio_{symbol}.pdf", mime="application/pdf")
